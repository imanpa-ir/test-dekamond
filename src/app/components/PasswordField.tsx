...
export default function PasswordField({ id, name, label, placeholder }: PasswordFieldProps) {
  ...
  return (
    <div className="transition-transform duration-200 hover:scale-[1.02]">
      <label htmlFor={id} className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">{label}</label>
      <div className="relative">
        ...
        />
        <button type="button" onClick={() => setShow(!show)} className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 transition-colors duration-200 focus:outline-none">
          {show ? "🙈" : "👁️"}
        </button>
      </div>
    </div>
